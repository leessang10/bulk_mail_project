...
import { convertMjmlToHtml, processMergeFields } from '../../../common/utils/mail.util';
...
export class TemplateService {
   ...
   * 새 메일 템플릿을 생성합니다.
   */
  async createTemplate(createTemplateDto: CreateTemplateDto): Promise<MailTemplate> {
   ...
   * 모든 메일 템플릿을 조회합니다.
   */
  async findAllTemplates(page = 1, limit = 20): Promise<{ templates: MailTemplate[], total: number }> {
  ...
  async processTemplate(templateId: string, mergeFields: MergeFields): Promise<string> {
    ...
    // 머지 필드 처리
    const processedMjml = this.processMergeFields(template.mjmlContent, mergeFields);
   ...
   */
  async previewTemplate(id: string, previewData: Record<string, string>): Promise<string> {
    const template = await this.findTemplateById(id);
    
    // 머지 필드 적용
    const processedMjml = this.processMergeFields(template.mjmlContent, previewData);
   ...
   * 템플릿을 업데이트합니다.
   */
  async updateTemplate(id: string, updateData: Partial<CreateTemplateDto>): Promise<MailTemplate> {
